...
for del=2:35
    delt=min(del,12);
    exr=1+(delt-2)*0.1;% social response factor for after delta
    
    P=P0;
    Q=popm;% initial male pop
    S=zeros(300,1);% total male pop over time
    S(1)=sum(Q);
    R=zeros(300,1);% total female pop
    R(1)=sum(popf);
    for t=2:300
        Pn=P;% prepare to update P
        Qn=Q;
        Qn(1)=0;
        Pn(1,:)=0;
        for i=2:100
            if i>res
                exr2=1+(delt-2)*0.05;
                % social response factor for no-child females
                if i+del>40 % cannot give birth after delta
                    exr2=1;
                end
                Pn(i,1)=sum(P(i-1,del+2:99))*brtrue(i-1)...
                    +P(i-1,100)*brtrue(i-1)*exr2...
                    +P(i-1,del+1)*brtrue(i-1)*exr; % give birth
                Pn(1,100)=Pn(1,100)+Pn(i,1)*xi; % new born female
                Qn(1)=Qn(1)+Pn(i,1)*(1-xi);
                for j=2:del+1
                    Pn(i,j)=P(i-1,j-1)*(1-drf(i-1));
                end
                for j=del+2:del+2
                    Pn(i,j)=P(i-1,j-1)*(1-drf(i-1)-brtrue(i-1)*exr);
                end
                for j=del+3:99
                    Pn(i,j)=P(i-1,j-1)*(1-drf(i-1)-brtrue(i-1));
                end
                Pn(i,100)=P(i-1,100)*(1-drf(i-1)-brtrue(i-1)*exr2);
            else
                Pn(i,1)=0;
                for j=2:99
                    Pn(i,j)=P(i-1,j-1)*(1-drf(i-1));
